...
class SimpleStyle:
    ...
    def __init__(self,
        ...
        try:
            if "font" in conf:
                if "default" in conf["font"]:
                    self.font_default = conf["font"]["default"]
                if "east-asia" in conf["font"]:
                    self.font_east_asia = conf["font"]["east-asia"]

                if "size" in conf["font"]:
                    if str(conf["font"]["size"]).isdigit():
                        self.font_size = float(conf["font"]["size"])
                    else:  # 如果是以中文形式给出的字号，进行转换
                        self.font_size = float(_zihao_to_pt(conf["font"]["size"]))
        except KeyError:
        ...
        # 颜色有指定时检查，不指定默认黑色
        if conf.get("font", {}).get("color") is not None:
            try:
                color = conf["font"]["color"]
                if type(color) != str or len(color) != 6:
                    raise TypeError
                # 尝试进行转换为16进制数，并且是否符合RGB大小
                hex_num = int(color, 16)
